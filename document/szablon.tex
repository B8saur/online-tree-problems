...
\author{
    ...
    Advisor: dr. Grzegorz Gutowski
}
\date{Kraków, Poland \\ September 2026}
\begin{document}
\maketitle
...
\section*{Abstract}
In this paper, we study fractional online cover problems on trees. In online fractional vertex cover problem we improve$^{[1]}$ the lower bound to $\frac{3 - \phi}{2}$. We describing methods used to find it with (brute force) linear programming. We also provide a direct proof of the achieved lower bound, as a template for future work.
\newline
We also consider the online fractional independent set problem and establish lower and upper bounds of $\frac{1}{2}$ and $\frac{3}{4}$, respectively.
\bigskip
\bigskip
\bigskip
...
We will only consider inequalities (as above) after even number of turns, which is a less strict requirement, but sufficient for our analysis.
...
In conclusion, we have just shown that the \textit{score} of an infinite vertex cover game on a line (described earlier) is at least $1 + \alpha = \frac{5 - \phi}{2} \approx \textbf{1.690983}$



\section{Independent Set}
Vertex Cover and Independent Set are opposites of each other. Online problem on the former exhibits non-trivial behavio. Therefore, maybe the latter is also interesting. As a starting point, we provide both bounds to Independent Set problem, defined similarly to Vertex Cover above.

\paragraph{Lower bound}
By that we mean the highest (known) \textit{score} such that there exists an algorithm that always achieves a score of at least that much.

Simple answer is $\frac{1}{2}$. We just assign $\frac{1}{2}$ to each vertex, getting a total sum of $\frac{n}{2}$ for $n$ vertices, which is a half of the maximum of $n$ (or $n-1$ if $n > 1$).


\paragraph{Upper bound}
By that we mean the lowest (known) \textit{score} such taht there exists an adversary against which no algorithm can get a score higher than that much.

Here, the value is $\frac{3}{4}$. The tree is just two edges with one common vertex, with an equivalent of Symmetry Breaking. If the first edge has vertices of values $x$ and $y$, $x + y \le 1$ and $x \le y$, we attach an edge to the vertex with value $y$. Then, we get $y + z \le 1$, where $z$ is the value of the third vertex. Now, $x \le \frac{1}{2}$, so the total value of all vertices is $x + y + z \le \frac{3}{2}$. And the optimal independent set has a value of $2$. Therefore the score is $\frac{3}{4}$.



\section{Open problems}
One of the goals of this paper is to describe tools that can be used to further improve bounds on Online Fractional Vertex Cover and Independent Set. Most likely none of the bounds found above are perfect, but we hope those methods will help change those values.



\section*{References}
[1] Katarzyna Kępińska, Online Fractional Vertex Cover with Edge Arrivals
